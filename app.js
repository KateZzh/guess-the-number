    ...
    for (let i = start; i < end; i++) {
      ...
      if (userNum >= start && userNum <= end) {
        if (userNum == rndNum_) {
          ...
          start = userNum;
          alert(
            `Вы ввели число ${userNum} и это слишком маленькое значение. Введите число от ${start} до ${end}`
          );
        } else if (userNum > rndNum_) {
          countOfIteration++;
          end = userNum;
          alert(
            `Вы ввели число ${userNum} и это слишком большое значение. Введите число от ${start} до ${end}`
          );
        }
      } else {
        userNum;
